...
from datasets.phase.halo_phase import PhaseDataset_Halo
...
def build_dataset(is_train, test_mode, fps, args):
    ...
    elif args.data_set == "halo":
        mode = None
        anno_path = None
        if is_train is True:
            mode = "train"
            anno_path = os.path.join(
                '/mnt/hdd1/ale/Surgformer/data/halo', "labels_pkl", mode, fps + "train.pickle"
            )
        elif test_mode is True:
            mode = "test"
            # args.data_path = "/jhcnas1/yangshu/data/AutoLaparo"
            anno_path = os.path.join(
                '/mnt/hdd1/ale/Surgformer/data/halo', "labels_pkl", mode, fps + "test.pickle"
            )
        else:
            mode = "val"
            # args.data_path = "/jhcnas1/yangshu/data/AutoLaparo"
            anno_path = os.path.join('/mnt/hdd1/ale/Surgformer/data/halo', "labels_pkl", mode, fps + "val.pickle")
