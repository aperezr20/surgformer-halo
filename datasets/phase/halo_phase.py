...
class PhaseDataset_Halo(Dataset):
    """Load video phase recognition dataset."""

    def __init__(
        self,
        anno_path="data/halo/labels_pkl/train/1fpstrain.pickle",
        data_path="data/halo",
        mode="train",  # val/test
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            if self.data_strategy == "online":
                try:
                    buffer, phase_labels, sampled_list = self._video_batch_loader(frames, frame_id, video_id, index, False)  # T H W C
                except:
                    breakpoint()
    ...
    def _create_depth_mask(self, depth, mask):
    ...
    def _video_batch_loader_for_key_frames(self, duration, timestamp, video_id, index, cut_black):
        ...
        for num, image_index in enumerate(sampled_list):
            try:
                ...
                case_image_name = os.path.join(os.path.basename(os.path.dirname(path)), os.path.basename(path))

                depth_base_path = '/mnt/hdd1/ale/Surgformer/data/halo/depth'
                mask_base_path = '/mnt/hdd1/ale/Surgformer/data/halo/masks'

                # mask_path = os.path.join(mask_base_path, case_image_name.replace('.png','.npz'))
                depth_path = os.path.join(depth_base_path,  case_image_name)

                # masks = np.load(mask_path)['masks']
                depth = cv2.imread(depth_path)
                # depth_ = depth[:, :, 0]
                
                # human_mask = masks[:, :, 5].astype(bool)
                # objects_mask = np.any(np.delete(masks, 5, axis=2), axis=2).astype(bool)

                # masked_human_depth = self._create_depth_mask(depth_, human_mask)

                # masked_object_depth = self._create_depth_mask(depth_, objects_mask)
                
                # combined_mask = np.any(masks, axis=2)

                # masked_bg_mask = np.logical_not(combined_mask).astype(bool)

                # masked_bg_depth = self._create_depth_mask(depth_, masked_bg_mask)

                # image_data = np.concatenate((masked_bg_depth, masked_human_depth, masked_object_depth), axis=2)
                image_data = depth 
...
def build_dataset(is_train, test_mode, fps, args):
    ...
    print("%s %s - %s : Number of the class = %d" % ("Halo", mode, fps, args.nb_classes))
